# SOC-PROJECT — A Working Security Operations Center Pipeline (Pure Python)

A complete, runnable SOC pipeline: **telemetry ingestion → log normalization → Sigma-style
detection rules → entity enrichment with threat intel → incident correlation → automated
playbook response → live triage dashboard → report & metrics.**
...
┌────────────────────┐   ┌──────────────────────┐   ┌───────────────────┐   ┌──────────────────┐
│ auth (Sysmon-ish)  │   │                      │   │ 29 Sigma-style    │   │ correlation →     │
│ web  (Apache comb.)│──▶│  parsers per source  │──▶│ rules + baseline  │──▶│ cases / incidents │
│ fw   (CSV)         │   │  → one normalized    │   │ beaconing (CoV)   │   │ playbooks (block, │
│ dns  (TSV)         │   │    event schema      │   │ enrichment (IP,   │   │ disable, isolate) │
│ proc (JSONL)       │   │  + parse-error count │   │ user, host, geo)  │   │ MTTD / MTTR       │
└────────────────────┘   └──────────────────────┘   └───────────────────┘   └──────────────────┘
                                                                                     │
                                                                          ┌──────────▼──────────┐
                                                                          │ live dashboard      │
...
Verified on the committed dataset: **10,443 normalized events → 29 alerts (24/29 rules fired) →
7 incidents → 16 simulated response actions**, ingest 1.6 s, detection 0.6 s (~18k events/s), 0
parse errors.

Run each command on its own line: `serve` and `live` keep running until you press Ctrl+C, so open a second PowerShell window for those. If you are already inside the project folder, drop the first `cd` line above.
...
```powershell
py -3 -m pip install -r requirements.txt      # py launcher → the Windows-installed Python
...
1. **Ingestion / normalization.** Four raw log formats (Sysmon-style JSON, Apache combined,
   ...
   `data/processed/events.jsonl` and point out the `parser_errors` counter — a real SOC always
...
2. **Detection engineering.** Rules live in YAML, Sigma-style: `selection` (OR of AND-groups),
   ...
   window), `technique` for MITRE ATT&CK mapping. Add a rule in 10 seconds, see it fire —
...
3. **Behavioral detection, not just signatures.** `beaconing.yaml` computes the coefficient of
   variation of inter-arrival times per (process, destination host) — no IOC list needed, it
...
4. **Enrichment & scoring.** Every alert carries entities (user risk, host criticality, IP
   ...
   (`soc/enrich.py`). Same alert on a domain controller scores higher than on a kiosk — that
   is how you justify priority to a manager.
5. **Correlation → incidents.** Alerts for one account/IP/host within a time window are stitched
   into one incident with a TTP narrative (recon → credential access → C2 → exfil → impact).
   Without this you drown in 40 alerts; with it you work 4 cases.
6. **Automated response.** Playbooks (from `config/settings.yaml`) run containment actions —
...
7. **Metrics.** MTTD (detection → alert) and MTTR (alert → case closed), FP rate from triage,
   ATT&CK technique coverage (`soc.py stats` prints rules that never fired — tuning is part of
   SOC work).
...
```
SOC-PROJECT/
├── soc.py                 CLI entry point (gen | ingest | rules | detect | cases | stats | report | run | demo | serve)
├── config/settings.yaml   paths, scoring weights, case thresholds, playbooks, severity map
├── rules/*.yaml           29 Sigma-style detection rules (+ rules/_template.yaml to extend)
├── intel/threat_intel.yaml small offline threat-intel feed (IPs, domains, categories)
├── soc/
│   ├── event_schema.py    normalized event schema + timestamp helpers
│   ├── parsers.py         jsonl / apache combined / csv / syslog-kv parsers, fault tolerant
│   ├── telemetry.py       synthetic multi-source telemetry with a full attack chain
│   ├── detection.py       rule engine: selectors, filters, thresholds, distinct, beaconing CoV
│   ├── enrich.py          entity resolution, geo/user/host intel, alert scoring
│   ├── cases.py           dedup, correlation, incident scoring, playbook execution
│   ├── dashboard.py       dependency-free live triage dashboard (port 8080)
│   ├── demo.py            `soc.py demo`: narrated incident walkthrough for presenting
│   └── geo.py             offline geolocation + RFC1918 test (see the TEST-NET gotcha below)
│   └── report.py          REPORT.md generator
├── tests/test_soc.py      20 unittests covering every stage, incl. the negative cases
│                          (benign input must produce no alerts, windows must be enforced)
...
3. Click `INC-…-001` → the kill chain line (recon → initial access → execution → C2 → exfil →
   impact) and the **automated response table with justifications**. Say: *"one case, 22 alerts"*.
4. Tab **Alert queue** → scroll to the *Score transparency* table: exactly why an alert scored 100
   vs 38. Then tab **ATT&CK & rules**: 5 idle rules are listed — that is tuning, not a bug.
5. Press **False positive** on the OWA alert, type a note → status flips to closed, `fp_count`
...
6. `cp data/telemetry/auth_windows.jsonl /tmp/x && printf 'not-a-log-line\n' >> data/telemetry/auth_windows.jsonl && python soc.py ingest`
   → the run prints `1 parse error`. Say: *"a SIEM that hides parse failures is a blind spot."*
...
- Swap the JSONL telemetry for a live tail: `soc.py ingest --follow` → same rules, real stream.
...
- Add a rule for DNS tunneling (bytes-per-query baseline) — the telemetry already contains it.
...
- "Internal vs external" uses an explicit RFC1918 list, **not** Python's
  ...
  203.0.113.0/24) as private — a real trap that silences every "external source" rule. There is a
...
- Detections are stateless per run (plus explicit window rules) — no long-lived session store,
